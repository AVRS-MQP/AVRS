...
def main():
    ...
    with sm:
        ...
        smach.StateMachine.add('set_calib', SetCalib(),
                               ...
                               remapping={'start':'start_signal',
                                          'current_tool':'tooling'})
        smach.StateMachine.add('camera_pose', CameraPose(),
                               transitions={'find_car':'find_car'},
                               remapping={'current_tool':'tooling'})

        sm_FindCar = Concurrence(outcomes=['open_flap'],
                                 default_outcome = 'open_flap',
                                 output_keys = ['car_model', 'has_pose'],
                                 outcome_map = {'succeeded':{'get_car_info':'succeeded',
                                                            'locate_flap':'succeeded'}})
        #pen concurrent sub-container
        with sm_FindCar:
            #Add states
        ...
        smach.StateMachine.add('find_car', FindCar(),
                               transitions={'get_car_info':'get_car_info',
                                            'locate_flap':'locate_flap'},
                               remapping={'current_tool':'tooling',
                                          'has_pose':'pose_found'})

        smach.StateMachine.add('flap', Flap(),
                                transitions={'flap_approach_pose':'flap_approach_pose',
                                            'find_port_pose':'find_port_pose'},
                                remapping={'car_model':'vehicle_model',
                                            'flap_is_open':'flap_open'})
        ...
        smach.StateMachine.add('open_flap', OpenFlap(),
                               ...
                               remapping={'flap_unlocked':'flap_unlocked',
                                         'open_error':'flap_error'})
